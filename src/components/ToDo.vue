      ...
      addItem(){
        if(this.entry !== ''){
          ...
          if(newEntry.favorite){
            // adiciona os novos itens na primeira posicao e substitui os antigos
            this.todoList.splice(0,0,newEntry);

          }else{
            // adiciona os novos itens na ultima posicao 
...
<style lang="scss" scoped>
  .container{
    ...
    .add-item{
      ...
      width:100%;
      height: 60px;
      background:rgba($color:#0000,$alpha: .3);
      border-radius:5px;
      overflow:hidden;
...
//componente +       
      ...
      input{
        ...
        color:#fffbfe;
        font-size: 1.6rem;
//conteudo dentro da caixa de texto
        &::placeholder {
          color:#fffbfe;
        }
// seta na caixa do texto
        :focus{
          outline:none;
        }
      }
// componente *
    ...
    .todo-list{
      padding-top: 20px;
        ...
        height: 60px;
        margin-bottom: 2px;
        background: rgba($color:#fffbfe,$alpha: .8);
        border-radius: 5px;
        overflow: hidden;
        opacity: 0;
    ...
    .complete-list{
      .item{
        position: relative;
        background: rgba($color:#fffbfe,$alpha: .4);
